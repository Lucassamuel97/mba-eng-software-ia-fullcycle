...
# Comentario em portugues: as chaves (Action/Action Input/Final Answer) devem ficar em ingles.
...
- If you output an Action, end the message right after Action Input.
- Never include Observation unless it was returned by a tool.
- When you output Final Answer, do not include any other fields.
- Always put the Action Input on its own line (no extra text on that line).
...
agent_chain = create_react_agent(
    llm, tools, prompt, stop_sequence=["\nObservation:", "\nFinal Answer:"]
)
