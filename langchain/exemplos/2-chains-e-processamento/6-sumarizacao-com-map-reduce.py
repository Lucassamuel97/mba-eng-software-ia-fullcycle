...
# MAP: cria um resumo curto para cada parte do texto.
partial_summaries = []
for idx, part in enumerate(parts, start=1):
    map_prompt = (
        "Resuma o trecho abaixo em portugues, em no maximo 2 linhas, "
        "preservando os pontos centrais.\n\n"
        f"Trecho {idx}:\n{part.page_content}"
    )
    map_result = llm.invoke(map_prompt)
    partial_summaries.append(map_result.content)

# REDUCE: combina os resumos parciais em um unico resumo final.
reduce_prompt = (
    "Voce recebeu resumos parciais de um texto maior. "
    "Gere um resumo final em portugues com no maximo 5 linhas.\n\n"
    "Resumos parciais:\n"
    + "\n".join(f"- {summary}" for summary in partial_summaries)
)

result = llm.invoke(reduce_prompt)
