...
## Fundamentos
* **[1-hello-world.py](exemplos/1-fundamentos/1-hello-world.py)**: Invoca o modelo Gemini diretamente com `ChatGoogleGenerativeAI` e imprime o texto retornado. Comando: `make run-hello-world`.
* **[2-init-chat-model.py](exemplos/1-fundamentos/2-init-chat-model.py)**: Inicializa um chat model via `init_chat_model`, abstraindo o provedor, e faz uma chamada simples. Comando: `make run-init-chat-model`.
* **[3-prompt-template.py](exemplos/1-fundamentos/3-prompt-template.py)**: Cria um `PromptTemplate` com variavel dinamica e faz a formatacao do texto. Comando: `make run-prompt-template`.
* **[4-chat-prompt-template.py](exemplos/1-fundamentos/4-chat-prompt-template.py)**: Monta mensagens `system` e `user` com `ChatPromptTemplate`, inspeciona as mensagens e invoca o modelo. Comando: `make run-chat-prompt-template`.

## Chains e Processamento

* **[1-iniciando-com-chains.py](exemplos/2-chains-e-processamento/1-iniciando-com-chains.py)**: Compoe um pipeline LCEL com `prompt | model` e executa via `invoke`. Comando: `make run-iniciando-com-chains`.
* **[2-chains-com-decorators.py](exemplos/2-chains-e-processamento/2-chains-com-decorators.py)**: Usa `@chain` para criar uma etapa customizada (quadrado) e encadeia com prompt e LLM. Comando: `make run-chains-com-decorators`.
* **[3-runnable-lambda.py](exemplos/2-chains-e-processamento/3-runnable-lambda.py)**: Demonstra `RunnableLambda` transformando string em inteiro e executando com `invoke`. Comando: `make run-runnable-lambda`.
* **[4-pipeline-de-processamento.py](exemplos/2-chains-e-processamento/4-pipeline-de-processamento.py)**: Pipeline de traducao + resumo com `PromptTemplate` e `StrOutputParser`. Comando: `make run-pipeline-de-processamento`.
* **[5-sumarizacao.py](exemplos/2-chains-e-processamento/5-sumarizacao.py)**: Faz chunking de texto longo e resume o conteudo em uma unica chamada. Comando: `make run-sumarizacao`.
* **[6-sumarizacao-com-map-reduce.py](exemplos/2-chains-e-processamento/6-sumarizacao-com-map-reduce.py)**: Implementa sumarizacao em duas fases (map e reduce) com LLM. Comando: `make run-sumarizacao-com-map-reduce`.
* **[7-pipeline-de-sumarizacao.py](exemplos/2-chains-e-processamento/7-pipeline-de-sumarizacao.py)**: Constroi pipeline LCEL completo de map-reduce com `RunnableLambda` e `StrOutputParser`. Comando: `make run-pipeline-de-sumarizacao`.

## Agentes e Ferramentas (Tools)

* **[1-agente-react-e-tools.py](exemplos/3-agentes-e-tools/1-agente-react-e-tools.py)**: Cria um agente ReAct com tools locais (calculadora e busca mock) e prompt customizado. Comando: `make run-agente-react-e-tools`.
* **[2-agente-react-usando-prompt-hub.py](exemplos/3-agentes-e-tools/2-agente-react-usando-prompt-hub.py)**: Usa prompt do LangChain Hub para ReAct e executa agente com tool mock. Comando: `make run-agente-react-usando-prompt-hub`.

## Gerenciamento de Memória e Contexto

* **[1-armazenamento-de-historico.py](exemplos/4-gerenciamento-de-memoria/1-armazenamento-de-historico.py)**: Adiciona memoria de conversa por sessao com `RunnableWithMessageHistory`. Comando: `make run-armazenamento-de-historico`.
* **[2-historico-baseado-em-sliding-window.py](exemplos/4-gerenciamento-de-memoria/2-historico-baseado-em-sliding-window.py)**: Limita o historico com sliding window usando `trim_messages` antes do prompt. Comando: `make run-historico-baseado-em-sliding-window`.

## Ingestão de Dados, Embeddings e Bancos de Dados Vetoriais

* **[1-carregamento-usando-WebBaseLoader.py](exemplos/5-loaders-e-banco-de-dados-vetoriais/1-carregamento-usando-WebBaseLoader.py)**: Carrega uma pagina web com `WebBaseLoader`, aplica chunking e imprime os trechos. Comando: `make run-carregamento-usando-webbaseloader`.
* **[2-carregamento-de-pdf.py](exemplos/5-loaders-e-banco-de-dados-vetoriais/2-carregamento-de-pdf.py)**: Carrega um PDF com `PyPDFLoader`, aplica chunking com `RecursiveCharacterTextSplitter` e mostra a quantidade de chunks. Comando: `make run-carregamento-de-pdf`.
* **[3-ingestion-pgvector.py](exemplos/5-loaders-e-banco-de-dados-vetoriais/3-ingestion-pgvector.py)**: Ingestao de chunks do PDF no PGVector com embeddings do Gemini e ids estaveis. Comando: `make run-ingestion-pgvector`.
* **[4-search-vector.py](exemplos/5-loaders-e-banco-de-dados-vetoriais/4-search-vector.py)**: Faz busca por similaridade no PGVector e imprime texto e metadados dos resultados. Comando: `make run-search-vector`.
